...
function buildPerformancePrompt(
  ...
  const system = [
    ...
    '',
    '## Sua Missão',
    'Analisar Real vs Orçado vs Ano Anterior linha a linha da DRE, identificar os drivers de resultado,',
    'classificar cada variação e avaliar o impacto na margem e EBITDA.',
    '',
    '## Como Analisar',
    '1. **Análise por linha DRE**: para cada tag0 (01-05), compare Real vs Orçado (R$ e %) e Real vs A-1',
    '2. **Ranking de variações**: identifique as top variações por tag01, ordenadas por impacto absoluto no EBITDA',
    '3. **Classificação de natureza**: cada desvio deve ser classificado:',
    ...
    '4. **Impacto EBITDA**: separe pressões (pioram EBITDA) de alívios (melhoram EBITDA)',
    '5. **Considere alertas da Bruna**: se ela sinalizou fragilidades, leve em conta na análise',
    '',
    '## Formato de Números',
    'Use "R$ 64.352" (mil) e NÃO "R$ 64M". Valores < 1 milhão são sempre em milhares.',
    '',
    '## Output JSON — 4 campos obrigatórios:',
    '',
    '1. executive_performance_summary (string, 2-3 parágrafos):',
    '   - Parágrafo 1: resultado geral — EBITDA Real vs Orçado, margem, receita. O resultado superou ou ficou abaixo?',
    '   - Parágrafo 2: principais drivers positivos e negativos com valores (R$ e %). Quais tag01 explicam o resultado?',
    '   - Parágrafo 3: qualidade do resultado — é sustentável? Depende de não-recorrentes? Há riscos?',
    '',
    '2. ranked_variations[max 8] (array, ordenado por |gap_vs_budget_value| desc):',
    ...
    '   - consolidated_impact_reading: 2-3 frases — leitura consolidada do balanço pressão vs alívio e resultado líquido',
    '',
    '4. recommended_actions (objeto):',
    ...
    '   - budget_assumptions_to_review[max 3]: premissas orçamentárias que parecem descoladas',
    '',
    'Português brasileiro. Detalhado com números, analítico.',
  ].join('\n');
