-- ═══════════════════════════════════════════════════════════════════
-- get_soma_tags_by_marca v3 — INCLUI transactions_manual
-- FIX: v2 não consultava transactions_manual, causando totais
-- menores no drill-down por marca (receitas negadas ficavam
-- excluídas de transactions MAS a versão corrigida em
-- transactions_manual não era somada).
--
...
AS $$
  ...
  -- 1a. Real (transactions) — exclui scenario='Original'
  SELECT
  ...
  -- 1b. Real (transactions_manual) — lançamentos manuais/corrigidos
  SELECT
    COALESCE(t.tag0, 'Sem Classificação') AS tag0,
    COALESCE(t.marca, '')                 AS marca,
    'Real'                                AS scenario,
