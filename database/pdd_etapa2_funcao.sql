-- ══════════════════════════════════════════════════════════════════════
-- PDD — ETAPA 2: Função automática completa
--
-- Lógica:
--   1. Soma receita das contas em pdd_contas (recurring='Sim', Real/Original)
--      Agrupado por mês + filial
--   2. Aplica share_pdd (% por marca)
--   3. valor_pdd = receita_bruta × share% / 100 × -1 (custo negativo)
--   4. UPSERT em pdd_log (auditoria)
--   5. UPSERT em transactions (chave_id determinística)
--      Tags via de-para da conta_contabil (cod_conta = '4.2.1.13.01.03')
--   6. Refresh dre_agg
--
-- Campos em transactions:
--   conta_contabil = '4.2.1.13.01.03'
--   tag0           = '04. SG&A'  (ou do de-para)
--   tag01/02/03    = via lookup em conta_contabil
--   vendor         = 'Planejamento Financeiro'
--
-- Padrão idêntico ao calcular_rateio_raiz_real()
-- ══════════════════════════════════════════════════════════════════════
...
--           tag01 da conta 4.2.1.13.01.03 deve mapear para '04. SG&A'
-- ══════════════════════════════════════════════════════════════════════

-- Busca a tag1 (=tag01) da conta_contabil e insere no tag0_map
-- Se a conta já tiver mapeamento, apenas atualiza
DO $$
DECLARE
  v_tag1 TEXT;
BEGIN
  ...
  IF v_tag1 IS NOT NULL THEN
    ...
    ON CONFLICT (tag1_norm) DO NOTHING;  -- não sobrescrever se já existe
  END IF;
END;
$$;
...
BEGIN
  ...
  receita_base AS (
    ...
    WHERE COALESCE(t.scenario, 'Real') IN ('Real', 'Original')
      ...
      -- Excluir lançamentos de PDD anteriores para não auto-referenciar
...
-- ══════════════════════════════════════════════════════════════════════
-- PASSO 4 — Verificar de-para antes de rodar
-- ══════════════════════════════════════════════════════════════════════

-- Confere as tags que serão usadas (via conta_contabil)
SELECT
  cod_conta,
  tag1 AS "tag01 (será usado)",
  tag2 AS "tag02 (será usado)",
  tag3 AS "tag03 (será usado)",
  ger,
  bp_dre
FROM conta_contabil
WHERE cod_conta = '4.2.1.13.01.03';
...
-- Conferir transactions inseridas (todos os campos)
SELECT
