-- ═══════════════════════════════════════════════════════════════════
-- FIX: Normalizar vendor case no drill-down DRE Gerencial
-- Problema: "Primasoft Informática" (Real) vs "PRIMASOFT INFORMÁTICA" (Orçado)
--   aparecem como linhas separadas no drill-down por FORNECEDOR
--
-- Solução:
--   1) get_dre_dimension: quando dimension='vendor', usa INITCAP(TRIM())
--      no SELECT e GROUP BY para unificar variações de case
--   2) get_soma_tags_by_vendor: filtro p_vendor case-insensitive
--   3) search_vendors: retorna INITCAP(TRIM()) deduplicado
--
...
-- ═══════════════════════════════════════════════════════════════════
-- 1. search_vendors — retorna vendors normalizados (INITCAP + TRIM)
-- ═══════════════════════════════════════════════════════════════════
...
AS $$
  SELECT DISTINCT vendor FROM (
    SELECT DISTINCT INITCAP(TRIM(vendor)) AS vendor FROM transactions
    WHERE vendor IS NOT NULL AND TRIM(vendor) <> ''
      AND (scenario IS NULL OR scenario = 'Real')
    UNION
    SELECT DISTINCT INITCAP(TRIM(vendor)) AS vendor FROM transactions_manual
    WHERE vendor IS NOT NULL AND TRIM(vendor) <> ''
      AND (scenario IS NULL OR scenario = 'Real')
    UNION
    SELECT DISTINCT INITCAP(TRIM(vendor)) AS vendor FROM transactions_orcado
    WHERE vendor IS NOT NULL AND TRIM(vendor) <> ''
    UNION
    SELECT DISTINCT INITCAP(TRIM(vendor)) AS vendor FROM transactions_ano_anterior
    WHERE vendor IS NOT NULL AND TRIM(vendor) <> ''
  ) AS all_vendors
...
-- ═══════════════════════════════════════════════════════════════════
-- 2. get_dre_dimension — normaliza vendor com INITCAP(TRIM())
--    Para dimension='vendor': GROUP BY INITCAP(TRIM(vendor))
--    Para outras dimensões: GROUP BY coluna como antes
-- ═══════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Normalizar vendor para unificar variações de case (ex: "PRIMASOFT" = "Primasoft")
  IF p_dimension = 'vendor' THEN
    v_dim_expr := 'INITCAP(TRIM(vendor))';
  ELSE
    v_dim_expr := format('%I', p_dimension);
  END IF;

  RETURN QUERY EXECUTE format(
    'SELECT
       COALESCE(CAST(%s AS text), ''N/A'') AS dimension_value,
       year_month,
     ...
     GROUP BY COALESCE(CAST(%s AS text), ''N/A''), year_month',
    v_dim_expr, v_dim_expr
  )
...
-- ═══════════════════════════════════════════════════════════════════
-- 3. get_soma_tags_by_vendor — filtro p_vendor case-insensitive
-- ═══════════════════════════════════════════════════════════════════
