...
def carregar_planilha():
    ...
    # ── normalizar nomes de colunas (remove espaços, converte para minúsculo) ──
    df.columns = df.columns.str.strip().str.lower()
    print_ok(f"Colunas encontradas: {', '.join(df.columns.tolist())}")
    ...
    for _, row in df.iterrows():
        ...
        record = {
            ...
            "tag01":         safe_str_title(row.get("tag1") or row.get("tag01"), 200),
            "tag02":         safe_str_title(row.get("tag2") or row.get("tag02"), 200),
            "tag03":         safe_str_title(row.get("tag3") or row.get("tag03"), 200),
            "vendor":        safe_str(row.get("vender") or row.get("vendor"), 500),
