-- ═══════════════════════════════════════════════════════════════════
-- fix_get_soma_tags_v5.sql
-- Adiciona coluna `month` (YYYY-MM) ao retorno para suportar visões
-- Mês e Cenário. Para A-1 os meses são alinhados ao ano do filtro.
-- ═══════════════════════════════════════════════════════════════════
...
AS $$
  ...
  -- 3. A-1 (transactions_ano_anterior — meses alinhados ao ano do filtro)
  SELECT
    COALESCE(tm.tag0, 'Sem Classificação')    AS tag0,
    COALESCE(t.tag01, 'Sem Subclassificação') AS tag01,
    'A-1'                                     AS scenario,
...
-- Testes rápidos:
-- SELECT * FROM get_soma_tags('2026-01','2026-12') ORDER BY tag0, tag01, scenario, month LIMIT 30;
-- SELECT scenario, month, COUNT(*), SUM(total) FROM get_soma_tags('2026-01','2026-12') GROUP BY scenario, month ORDER BY scenario, month;
